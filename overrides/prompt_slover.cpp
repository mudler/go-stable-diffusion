...
PromptSlover::PromptSlover(string assets_dir)
{
	// ����CLIPģ��
	...
	std::filesystem::path params = std::filesystem::path(assets_dir) / std::filesystem::path("FrozenCLIPEmbedder-fp16.param");
	std::filesystem::path model = std::filesystem::path(assets_dir) / std::filesystem::path("FrozenCLIPEmbedder-fp16.bin");

	net.load_param(params.string().c_str());
	net.load_model(model.string().c_str());

	// ��ȡtokenizer�ֵ�
	std::ifstream infile;
	std::filesystem::path v = std::filesystem::path(assets_dir) / std::filesystem::path("vocab.txt");
	std::string pathname = v.string();
