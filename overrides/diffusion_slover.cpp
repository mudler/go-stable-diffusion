...
DiffusionSlover::DiffusionSlover(int h, int w, int mode, string assets_dir)
{
	...
	if (h == 512 && w == 512)
		param_file = "UNetModel-512-512-MHA-fp16-opt.param";
	else if (h == 256 && w == 256)
		param_file = "UNetModel-256-256-MHA-fp16-opt.param";
	else
	{
	...
	ifstream in(sigma_path.string(), ios::in | ios::binary);
	in.read((char*)&log_sigmas, sizeof log_sigmas);
	in.close();
}

void DiffusionSlover::generate_param(int height, int width, string assets_dir)
{
	string line;

	std::filesystem::path decoder = std::filesystem::path(assets_dir) / std::filesystem::path("UNetModel-base-MHA-fp16.param");
	std::filesystem::path decoder_out = std::filesystem::path(assets_dir) / std::filesystem::path("tmp-UNetModel-" + std::to_string(height) + "-" + std::to_string(width) + "-MHA-fp16.param");

	ifstream diffuser_file(decoder.string().c_str());
	ofstream diffuser_file_new(decoder_out.string().c_str());
