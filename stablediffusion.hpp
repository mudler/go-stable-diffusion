// This is an adaptation of https://github.com/fengwang/Stable-Diffusion-NCNN/blob/main/stable_diffusion.hpp
// Credits goes to fengwang and EdVince
...
inline static ncnn::Mat decoder_solver( ncnn::Mat& sample, string assets_dir )
{
    ncnn::Net net;
    {
        ...
        std::filesystem::path param_path = std::filesystem::path(assets_dir) / std::filesystem::path("AutoencoderKL-fp16.param");
...
inline static ncnn::Mat diffusion_solver( int seed, int step, ncnn::Mat& c, ncnn::Mat& uc , string assets_dir)
{
    ncnn::Net net;
    {
        net.opt.use_vulkan_compute = false;
        ...
        net.opt.use_fp16_arithmetic = true;
        net.opt.use_packing_layout = true;
        std::filesystem::path param_path = std::filesystem::path(assets_dir) / std::filesystem::path("UNetModel-fp16.param");
        std::filesystem::path model_path = std::filesystem::path(assets_dir) / std::filesystem::path("UNetModel-fp16.bin");
	    net.load_param(param_path.string().c_str());
	    net.load_model(model_path.string().c_str());
    }
...
inline static ncnn::Mat esr4x( ncnn::Mat& input, string assets_dir )
{
    ncnn::Net net;
    {
        net.opt.use_vulkan_compute = false;
        ...
        net.opt.use_packing_layout = true;
        std::filesystem::path param_path = std::filesystem::path(assets_dir) / std::filesystem::path("RealESRGAN_x4plus_anime_6B.fp32-sim-sim-opt.param");
        std::filesystem::path model_path = std::filesystem::path(assets_dir) / std::filesystem::path("RealESRGAN_x4plus_anime_6B.fp32-sim-sim-opt.bin");
	    net.load_param(param_path.string().c_str());
	    net.load_model(model_path.string().c_str());
    }
    ncnn::Extractor ex = net.create_extractor();
...
inline static std::pair<ncnn::Mat, ncnn::Mat> prompt_solver( std::string const& prompt_positive, std::string const& prompt_negative, string assets_dir )
{
    ...
    ncnn::Net net;
    {
        ...
        std::filesystem::path param_path = std::filesystem::path(assets_dir) / std::filesystem::path("FrozenCLIPEmbedder-fp16.param");
        std::filesystem::path model_path = std::filesystem::path(assets_dir) / std::filesystem::path("FrozenCLIPEmbedder-fp16.bin");
	    net.load_param(param_path.string().c_str());
	    net.load_model(model_path.string().c_str());
        // 读取tokenizer字典
        std::ifstream infile;
